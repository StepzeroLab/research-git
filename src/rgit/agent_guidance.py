...
def render_global_block(mode: str = "default") -> str:
    ...
    return (
        ...
        "- Already committed the work? Capture it from history instead: "
        "`rgit capture --commit HEAD` (or `rgit capture --range A..B` for "
        "several commits) — after a commit the working tree is clean, so a "

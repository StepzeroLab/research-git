...
def render_global_block(mode: str = "default") -> str:
    ...
    return (
        ...
        "  - If the work was committed as one coherent commit, run "
        "`rgit capture --commit HEAD`.\n"
        "  - If the work spans multiple small commits, run "
        "`rgit capture --range <base>..HEAD`.\n"
        "  - Skip mechanical formatting, dependency churn, generated files, "
        "or changes with no reusable research/code idea.\n"
        "- Use the `rgit-capture` skill to segment pending captures into "
